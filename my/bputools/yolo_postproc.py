...
def get_yolov3_darknet53_config():
    yolov3_darknet53_config = EasyDict()
    yolov3_darknet53_config.ANCHORS = np.array([
        1.25, 1.625, 2.0, 3.75, 4.125, 2.875, 1.875, 3.8125, 3.875, 2.8125,
        3.6875, 7.4375, 3.625, 2.8125, 4.875, 6.1875, 11.65625, 10.1875
    ]).reshape((3, 3, 2))
    yolov3_darknet53_config.STRIDES = np.array([8, 16, 32])
    yolov3_darknet53_config.NUM_CLASSES = 80
    yolov3_darknet53_config.CLASSES = MSCOCODetMetric.class_names
    yolov3_darknet53_config.INPUT_SHAPE = (416, 416)
    return yolov3_darknet53_config
...
def modelout2predbbox(model_output):
    yolov3_darknet53_config = get_yolov3_darknet53_config()
    num_classes = yolov3_darknet53_config.NUM_CLASSES
    anchors = yolov3_darknet53_config.ANCHORS
    num_anchors = anchors.shape[0]
    strides = yolov3_darknet53_config.STRIDES
    ...
    pred_sbbox = yolo_decoder(pred_sbbox, num_anchors, num_classes, anchors[0], strides[0])
    pred_mbbox = yolo_decoder(pred_mbbox, num_anchors, num_classes, anchors[1], strides[1])
    pred_lbbox = yolo_decoder(pred_lbbox, num_anchors, num_classes, anchors[2], strides[2])
